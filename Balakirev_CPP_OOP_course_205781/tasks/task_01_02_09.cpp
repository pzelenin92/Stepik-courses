...
находятся в диапазоне [-100; 100]. Если хотя бы одна координата выходит из этого диапазона, то поля x, y, z
меняться не должны (присваивать ничего не нужно).

В функции main создайте через метод new два объекта класса Vector3D, на которые должны вести указатели
 ptr_v1, ptr_v2. Занесите в эти объекты координаты:
