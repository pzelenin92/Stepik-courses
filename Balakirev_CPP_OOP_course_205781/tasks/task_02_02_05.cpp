...
То есть, из строки выбираются первые два значения, записанные через пробел(ы) и приводятся к типу int. Если данные
оказываются не числовыми, то подставлять значение 0. Например:
